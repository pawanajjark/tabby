...
function routeFields(state: IdentityFeatureState): string {
  switch (state.route) {
    ...
    case 'join-home':
      return `<section class="identity-choice-section"><p class="identity-kicker">AVAILABLE HOMES</p><div class="identity-choice-list">${state.homes.length ? state.homes.map(home => `<button type="button" data-identity-home="${home.id}" class="identity-choice${home.active ? ' is-current' : ''}"><strong>${escapeHtml(home.name)}</strong><span>${escapeHtml(`${home.label}, ${home.residenceName}`)}</span><small>${home.active ? 'Current home' : 'Click to join this home'}</small></button>`).join('') : '<p class="identity-empty">No homes are available yet. Go back and create one.</p>'}</div></section><p class="identity-trust-note"><span aria-hidden="true">◇</span>Choose a home to join. Its shared Home shelf becomes available to you; private conversations remain separate.</p>`;
